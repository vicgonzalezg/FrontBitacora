        ...
        if perfil['perfil'] == 1:
            if request.method == 'POST':
                ...
                response = requests.put(
                    modProceso, json=modificarProcesoJson, headers=headers)
...
def finProceso(request, id):
    try:
        ...
        if perfil['perfil'] == 1:
            ...
            response = requests.put(
                urlProcesos, json=finProcesoJson, headers=headers)
            #print(response)
            if response.status_code == 200:
                # mensaje para avisar al front que se modifico el proceso.
        ...
        if perfil['perfil'] == 1:
            ...
            if request.method == 'POST' and perfil['perfil'] == 1:
                ...
                response = requests.post(
                    urlCrearUsuarios, headers=headers, json=crearUsuariosJson)
                print(response)
                if response.status_code == 201:
                    ...
                    messages.error(
                        request, 'Hubo un problema al crear el usuario.')
        ...
        if perfil['perfil'] == 1:
            if request.method == 'POST':
                ...
                urlModUsuarios = 'http://127.0.0.1:8001/usuarios/' + \
                    str(id) + '/'
                response = requests.put(
                    urlModUsuarios, headers=headers, json=modificaUsuarioJson)
        ...
        if perfil['perfil'] == 2:
            if request.method == 'POST':
                ...
                if response.status_code == 200:
                    ...
                    return redirect('infoProcCoach', id)
                else:
                    messages.error(
    ...
    try:
        ...
        if perfil['perfil'] == 2 or perfil['perfil'] == 1:
            ...
            css_url = os.path.join(
                settings.BASE_DIR, 'frontProject/static/css/bootstrap.css')
        ...
        if perfil['perfil'] == 3:
            if request.method == 'POST':
                ...
                modPlanAccion = 'http://127.0.0.1:8001/procesos/' + \
                    str(id) + '/'
                response = requests.put(
                    modPlanAccion, json=modificarPlanJson, headers=headers)
        ...
        if perfil['perfil'] == 3:
            if request.method == 'POST':
                ...
                modRespAvances = 'http://127.0.0.1:8001/sesiones/' + \
                    str(id) + '/'
                response = requests.put(
                    modRespAvances, json=modificarRespAvancesJson, headers=headers)
                
                print(modRespAvances)
                print(modificarRespAvancesJson)
                if response.status_code == 200:
